...
impl Terminal {
    ...
    fn print_zero_width(&mut self, ch: char) {
        // The mark belongs to the last glyph before the cursor: the last
        // column when a wrap is pending, otherwise the cell just left of the
        // cursor. At column 0 there is no such glyph on this row; the mark
        // is dropped (display width zero, so geometry is unaffected).
        let col = if self.cursor.col >= self.cols {
            self.cols - 1
        ...
        self.buffer.append_zero_width((col, self.cursor.row), ch);
    }
    ...
    pub fn dump(&self) -> Vec<Function> {
        ...
        if self.cursor.col >= self.cols {
            ...
            if occupancy == Occupancy::Single {
                ...
                // The reprint wiped the cell's zero-width extras; re-attach.
                for &zw in last_cell.zero_width() {
                ...
                for &zw in prev_cell.zero_width() {
                    funs.push(Function::Print(zw));
                }
            }
        }
...
fn dump_cells(cells: &[Cell], funs: &mut Vec<Function>) {
    ...
    while i < cells.len() {
        ...
        while i + run_len < cells.len()
            ...
            && !cells[i + run_len].has_zero_width()
        {
            run_len += 1;
        }
