...
impl Line {
    ...
    /// Attach a zero-width character to the cell holding the glyph at `col`.
    /// If `col` addresses a wide character's tail, the mark lands on its head.
    pub(crate) fn append_zero_width(&mut self, mut col: usize, ch: char) {
        ...
        self.cells[col].push_zero_width(ch);
    }
    ...
    pub fn chars(&self) -> impl Iterator<Item = char> + '_ {
        self.cells
            .iter()
            .filter(|c| c.occupancy() != Occupancy::WideTail)
            .flat_map(|c| std::iter::once(c.char()).chain(c.zero_width().iter().copied()))
    }
