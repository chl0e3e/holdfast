...
impl Builder {
    ...
    // Upstream exposes `enable_webtransport` only on the *server* builder, so
    // an h3 client structurally advertises SETTINGS_ENABLE_WEBTRANSPORT=0.
    // h3-webtransport's server reads the peer's settings and refuses the
    // session with H3_SETTINGS_ERROR ("webtransport is not supported by
    // client") - see h3-webtransport-0.1.2/src/server.rs:90. A WebTransport
    // client therefore cannot exist without this. Good upstream PR candidate.
    /// Advertise SETTINGS_ENABLE_WEBTRANSPORT to the server.
