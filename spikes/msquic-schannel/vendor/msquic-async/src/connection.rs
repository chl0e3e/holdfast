...
impl Connection {
    ...
    // The ADR 0008 channel binding needs the peer leaf certificate off the
    // live connection, via GetParam(PARAM_TLS_SCHANNEL_CONTEXT_ATTRIBUTE_W).
    // Upstream keeps the msquic handle entirely private. This is the only
    // change to this vendored copy.
    /// Raw msquic connection handle, for `GetParam`.
    ///
    /// # Safety
    /// The caller must not close the handle; it stays owned by this
    /// `Connection` and is only valid while this value is alive.
    pub unsafe fn msquic_handle(&self) -> msquic::ffi::HQUIC {
        unsafe { self.0.msquic_conn.as_raw() }
    }
