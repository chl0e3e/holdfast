...
// ---- holdfast spike patch (spikes/msquic-schannel) -----------------------
// Upstream exposes no route to the connection it owns: `conn` is pub(crate)
// and the Deref to msquic::Connection lives on the private ConnHandle. The
// ADR 0008 channel binding needs the peer leaf certificate off the live
// connection, so expose the handle. Nothing else in this vendored copy is
// modified; see the spike README for why we forked rather than upstreamed.
impl Connection {
    /// The underlying msquic connection, for `GetParam` of the Schannel peer
    /// certificate (`PARAM_TLS_SCHANNEL_CONTEXT_ATTRIBUTE_W`).
    pub fn msquic_connection(&self) -> &msquic::Connection {
        use std::ops::Deref;
        self.conn.deref().deref()
    }
}
// ---- end holdfast spike patch -------------------------------------------
