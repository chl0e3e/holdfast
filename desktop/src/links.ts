...
// Ported from web/src/client/links.ts (the reference implementation); the
// only divergence is that navigation goes through an injected opener —
// Tauri's webview has no working window.open, so the app passes
// ipc.openExternal (Rust re-validates the scheme and hands the URL to the
// OS default browser).
//
